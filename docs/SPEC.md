...
| Cierre tarjetas Galicia | CIERRE\_GALICIA=5 (Visa cierra día 5, Master día 6). Se usa para calcular primera cuota en compras con tarjeta | 2026-03-01 |
| Decimales estandarizados | Todos los montos se muestran con exactamente 2 decimales y coma como separador decimal (formato argentino). Bot: `toLocaleString('es-AR', {min/maxFractionDigits: 2})`. Dashboard: `{:,.2f}` con swap separadores. Cuotas: `Math.round(x*100)/100` en vez de `Math.round(x)` | 2026-03-01 |
| Menú carga manual /gasto | Wizard interactivo paso a paso (6 pasos: categoría, descripción, monto, moneda, método pago, tipo). Usa InlineKeyboards con callbacks `mg_*`. Al final reutiliza `tx_ok`/`tx_no` existentes. Botón "Registrar" del menú muestra submenu: Nuevo gasto / Gastos fijos | 2026-03-01 |
| Dashboard presupuestos 13 cats | Índices de parseo actualizados para 13 categorías ARS (incluye Seguros e Impuestos). Sección Oriana USD agregada al segmented control. `cargar_presupuesto_usd()` retorna dict con moises y oriana | 2026-03-01 |
...
| Fijos resilientes a reinicios | Si el bot se reinicia entre el envío de la notificación y el click del usuario, `reconstructPendingFijos()` reconstruye el estado desde el Sheet en vez de devolver "Expirado". Los callbacks fijos\_ok, fijos\_edit, fijos\_pick y fijos\_back son auto-contenidos | 2026-04-14 |
...
| Dashboard auto-update | B4 (mes) y B5 (año) son valores estáticos con dropdown (meses 1-12, años 2026-2035) para revisar meses pasados. El bot los resetea al período corriente en el cron `0 0 1 * *` (rollover de mes) vía `setDashboardPeriod()` en sheets.js. Guarda al arranque (`syncDashboardPeriod()` en scheduler.js, +7s): si quedaron en un mes anterior al actual los actualiza; no pisa navegación manual del mes en curso ni futuro | 2026-06-29 |
...
| Comando /proyeccion | Proyección per-user para mes futuro. Fijos y cuotas son exactos del Sheet (compartidos al 50%); variables = promedio de transacciones reales del usuario (últimos 3 meses, ARS/USD) − fijos − carga de cuotas histórica (`avgTransactionSpend`, evita el estimado TC×1.5 que inflaba gastos). El sobrante varía por mes según las cuotas activas reales de ese mes (ver "Sobrante proyectado por carga de cuotas real"). USD: quedaDeel − fijos USD − variables USD. Default: próximo mes. Módulo: `bot/src/projection.js` | 2026-05-29 |
| Menú depurado + fix /tarjeta | Menú persistente reducido a 9 botones (Registrar, Balance, Resumen, Últimas, Flujo, Próximo, Proyección, Saldar, Ayuda). Se sacaron Tarjeta, Cuotas, Borrar, Crypto, Inversiones y Ahorro (siguen disponibles tipeando el comando). Se agregó botón Proyección. Bug /tarjeta: el label `[tarjeta]` con corchetes rompía el Markdown legacy de Telegram (`can't parse entities`) cuando había >1 tarjeta en el mes; se cambió a paréntesis | 2026-06-29 |
| Fix /tarjeta mensaje largo | Con muchas transacciones, el detalle superaba el límite de 4096 chars de Telegram (`message is too long`). Helper `replyChunked()` parte el texto en mensajes de ≤3900 chars cortando en saltos de línea (no rompe Markdown). Aplicado a /tarjeta | 2026-06-29 |
| Fix doble conteo de ahorro en proyecciones | `avgTransactionSpend()` (projection.js) sumaba TODAS las transacciones ARS, incluyendo las de categoría "Ahorro / Inversión" (la plata que se aparta a ahorro). Eso inflaba el gasto promedio y, como la meta de ahorro se resta aparte, el ahorro se descontaba dos veces → `/puedo` y `/proyeccion` daban sobrante artificialmente bajo (ej: $745k en vez de $1.211.706) y veredictos ❌ falsos. Fix: excluir `CATEGORIAS_POSITIVAS` del promedio. Constante movida a `src/constants.js` (estaba duplicada en index.js y scheduler.js) | 2026-06-29 |
| Ahorro vuelve al menú | Se agregó botón **💎 Ahorro** al menú persistente (ahora 10 botones, emparejado con Ayuda). Se había sacado al depurar el menú, pero `/ahorro` pasó a ser acción frecuente (registrar aportes). `cmdAhorro` agregado a `CMD_HANDLERS` | 2026-06-30 |
| Aporte de ahorro desde /ahorro + meta multi-moneda | `/ahorro` tiene botón **"📥 Registré aporte"**: elegís cuenta (Deel USD / ARS Banco) y monto → en un solo paso (1) crea una transacción `Ahorro / Inversión` (`Aporte ahorro`, método `Deel USD`/`Banco`, tipo `Individual {quien}`) que alimenta la meta del mes y (2) **suma** el monto al saldo de esa cuenta en la hoja Ahorro (lee saldo actual + aporte, registra en historial). Un solo input, ambos lados sincronizados — antes había que cargar la transacción y aparte actualizar el saldo. Disponible para Moises y Oriana. La meta es **una sola y en ARS**: aportes USD cuentan convertidos por TC (`ahorradoEnArs()` en `constants.js`, usado por `checkSavingsAlert` y `/meta`). Decisión del usuario: NO medir la meta contra el saldo total de `/ahorro` (stock) porque revaluación de crypto/inversiones/TC contaminaría el "ahorrado del mes" (flujo) | 2026-06-30 |
| Transferencia Mamá en pesos | Moises le da a su madre **pesos** equivalentes a 200 USD según la cotización del mes, no dólares de su bolsillo USD. Cargarla como gasto USD hacía que `/proyeccion` diera **sobrante USD negativo** (−184,83): el bloque USD compara `quedaDeel` (lo que queda sin convertir, ~183 USD) contra los gastos USD, y los 200 del giro lo hundían. Fix sin código: en Gastos Fijos fila 32, moneda → `ARS`, método → `Banco ARS`, y monto = fórmula `=200*IF(INDEX(Ingresos!$E$3:$E$14;MONTH(TODAY()))>0;INDEX(...);LOOKUP(2;1/(...<>"");...))` (200 × TC Moises del mes en curso, con fallback al último TC cargado). Se recategorizaron las transferencias ya registradas (mar–jun 2026) de USD a ARS con el monto en pesos de cada mes, para efecto inmediato en `avgTxUsd`. Resultado: sobrante USD +15. Caveat: no usar "✏️ Editar monto" en ese ítem (sobrescribe la fórmula). **Fragilidad residual**: el bloque USD sigue midiendo `quedaDeel − gastosUsd`; queda apenas positivo | 2026-06-29 |
| Fix #N/A en fórmula Transferencia Mamá | La fórmula de C32 usaba `LOOKUP(2;1/(Ingresos!$E$3:$E$14<>"");Ingresos!$E$3:$E$14)` como fallback al último TC. Ese truco tira `#N/A` en este Sheet (Google Sheets no tolera los `#DIV/0!` del rango en el LOOKUP). No se notaba porque solo se ejecutaba el fallback cuando el mes en curso no tenía TC cargado: en julio 2026 (E9 vacía) INDEX dio 0 → cayó al LOOKUP roto → `#N/A`. Fix: reemplazar el fallback por `INDEX(Ingresos!$E$3:$E$14;MAX(FILTER(ROW(Ingresos!$E$3:$E$14)-ROW(Ingresos!$E$3)+1;Ingresos!$E$3:$E$14<>"")))` (posición del último TC no vacío). C32 vuelve a dar 268.000 (200 × 1340, último TC = Jun). Fórmula completa aplicada directo en la celda vía API | 2026-07-01 |
| Fix columna "Registrado" (H) en Gastos Fijos | La columna H (`✅ Sí`/`❌ No`) es un COUNTIFS que marca si el gasto fijo ya tiene transacción en el mes/año en curso (la genera `extendSheetLimits()` para H21:H101). Tres celdas habían perdido la fórmula por **edición manual de la hoja**: H29 (GearUp Portal) quedó con el valor estático `✅ Sí` (mostraba ✅ sin transacción de julio), y H36/H37 (IG verificado, Youtube) quedaron **vacías** (mostraban blanco pese a estar cargadas en julio). Fix: reescribir la fórmula COUNTIFS (rango `$C$2:$C$7980`, igual que las demás filas vivas) en H29/H36/H37 vía API. **Fragilidad conocida**: insertar/pegar filas o tipear en la columna H a mano borra la fórmula → los estados quedan clavados/en blanco. Para restaurar en masa: re-correr `extendSheetLimits()` (reescribe H21:H101; ojo que usa rango 5000, no 7980). El resto de los ❌ eran correctos: esos gastos genuinamente no estaban cargados en julio | 2026-07-01 |
| Sobrante proyectado por carga de cuotas real | Antes el sobrante de `/proyeccion` y `/puedo` era **plano** (idéntico para todo mes): `variablesArs` restaba la carga de cuotas del mes objetivo a un promedio histórico (`avgTxArs`) que ya la incluía, así que el término se cancelaba y quedaba `incomeArs − avgTxArs`. Nunca reflejaba el alivio al terminar las cuotas vigentes. Fix por **delta de carga de cuotas**: nueva `cuotaLoadForMonth(cuotasRaw, userId, month, year)` calendario-based (cuota activa si `0 ≤ monthsDiff(primeraCuota) < cuotasTotales`; solo ARS; compartido 50%) — NO usa `getPendingCuotasForMonth` (da $0 para meses pasados). `buildProjection` calcula `cuotaLoadHist` (promedio de la carga en los mismos 3 meses históricos) y `variablesArs = max(0, avgTxArs − fijosArs − cuotaLoadHist)`. Resultado: el sobrante de cada mes refleja su carga de cuotas REAL (Moises: Ago 2026 $848k pico, Feb 2027+ $1.49M sin cuotas). **Caveat**: solo proyecta la carga de cuotas CONOCIDA; no predice cuotas futuras nuevas; el alivio aparece solo si no se siguen sumando cuotas. `cuotasRaw` se pasa vía `deps` desde `cmdProyeccion` y `simulateAffordability` | 2026-06-29 |
...
| `/ahorro` | Ver ahorro total (Deel USD + ARS Banco + Crypto + Inversiones). Botones: registrar aporte (crea transacción de ahorro + suma al saldo) y actualizar saldos líquidos |
...
| `/proyeccion [mes]` | Proyección per-user para mes futuro: fijos (individuales + 50% compartidos), cuotas activas, variables estimados. El sobrante refleja la carga de cuotas real de cada mes (sube al terminar las cuotas vigentes). ARS + USD. Default: mes siguiente. Reemplaza a `/sobrante` (eliminado) |
...
La meta de ahorro está siempre en ARS (Moises $600.000, Oriana $200.000; no hay meta USD). Como el ahorro real puede ser en USD (Deel USD), el progreso se calcula con `ahorradoEnArs()` (`constants.js`): suma aportes ARS tal cual + aportes USD convertidos por el último TC (`getLastTC()`). Mismo helper usado por `/meta`. Si `tc <= 0`, los aportes USD no se cuentan (evita inflar con TC inválido).
...
- **Fase 7: Refinamiento** — En progreso. Ya implementados: `/tarjeta`, `/registrar_fijos`, recordatorio gastos fijos, tarjetas de crédito específicas, cuotas de tarjeta, rangos expandidos, parsing locale, filtros robustos, auto-fijos diario, alertas de presupuesto, resumen semanal, alertas de ahorro invertidas, batch write para gastos fijos, "Pagado por" en Gastos Fijos, Dashboard auto-update con dropdowns, `/gasto` (wizard interactivo), decimales estandarizados a 2 posiciones
- **Fase 8: Dashboard Streamlit** — Funcionando en Streamlit Community Cloud con viewer auth. Fix: presupuestos con 13 categorías ARS + Oriana USD
